...
public class Test15 extends Base{
    ...
    public void practiceFormSecond() {
        ...
        ((JavascriptExecutor) driver).executeScript("scroll(0,300)");
            ...
            WebElement cardAutoComplete = driver.findElement(By.xpath("//div[@class='element-list collapse show']//li[@id='item-1']"));
            action.click(cardAutoComplete).build().perform();


            WebElement autocompleteInput = wait.until(ExpectedConditions.presenceOfElementLocated(By.id("autoCompleteMultipleInput")));
            autocompleteInput.sendKeys("o");

            WebElement colorsMenu = wait.until(ExpectedConditions.presenceOfElementLocated(By.xpath("//div[contains(@class, 'auto-complete__menu-list')]")));
            String[] threeColor = {"Yellow", "Voilet", "Indigo"};

            List<String> colorsToCheck = Arrays.asList(threeColor);
            List<WebElement> colorsEl = colorsMenu.findElements(By.xpath("//div[contains(@class, 'auto-complete__option')]"));
